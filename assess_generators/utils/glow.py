"""
GLOW for CIFAR-10. Code adapted from repo: https://github.com/y0ast/Glow-PyTorch
"""
...
class Glow(nn.Module):
    ...
    def prior(self, data=None, y_onehot=None, batch_size=32):
        if data is not None:
...
# load the GLOW model from checkpoint
def load_glow(output_folder, checkpoint_name):
    ...
    s = torch.load(output_folder + checkpoint_name)
...
def generate_glow(glow, batch_size=32):
    y_onehot = None  # no conditioning
    temperature = 1  # temperature parameter

    mean, logs = glow.prior(torch.zeros((batch_size, 1)))  # prior data for shape only
    z = torch.normal(mean, torch.exp(logs) * temperature)
    fake_imgs_glow = glow(z=z, y_onehot=y_onehot, temperature=temperature, reverse=True)

    # the clamp referenced from the postprcess functionality of the github repo
    fake_imgs_glow = torch.clamp(fake_imgs_glow, -0.5, 0.5)
    fake_imgs_glow = fake_imgs_glow * 2
    return fake_imgs_glow
